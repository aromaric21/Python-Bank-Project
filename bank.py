...
class Bank:
    ...
    def deposit(self, amount):
        temp = db_query(f"SELECT balance,account_number FROM customers {self.__username} WHERE username = '{self.__username}';")
    ...
    def withdraw(self, amount):
        temp = db_query(f"SELECT balance FROM customers WHERE username = '{self.__username}';")
        if amount > temp[0][0]:
            print("Insuficient balance please money")
        else:
